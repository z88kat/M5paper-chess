...
void key_continue_cb(epdgui_args_vector_t &args) {
  Frame_Base *frame = EPDGUI_GetFrame("Frame_Continue");
  if (frame == NULL) {
    frame = new Frame_Continue();
    EPDGUI_AddFrame("Frame_Continue", frame);
  }
  EPDGUI_PushFrame(frame);
  *((int *)(args[0])) = 0;
}
...
/**
 * @brief Draw the name of the applications on the home screen
 */
void Frame_Main::AppName(m5epd_update_mode_t mode) {
  ...
  // move to position, x, y (down from top)
  _names->pushCanvas(0, 186, mode);

  _names->fillCanvas(0);

  // int32_t poX, int32_t poY
  _names->drawString("Storage", 20 + 46, 16);            // col 1
  _names->drawString("Compare", 20 + 46 + 136, 16);      // col 2
  _names->drawString("Home", 20 + 46 + 2 * 136, 16);     // col 3
  _names->drawString("LifeGame", 20 + 46 + 3 * 136, 16); // col 4

  // Push the above names to the canvas at x == 0, y == 186
  _names->pushCanvas(0, 337, mode);

  _names->fillCanvas(0);

  _names->drawString("Continue", 20 + 46, 16);
  _names->drawString("New Game", 20 + 46 + 136, 16);     // col 2
...
/**
 * @brief Draw the status bar on the home screen (top of the display)
 */
void Frame_Main::StatusBar(m5epd_update_mode_t mode) {
  ...
  _bar->drawString("M5PaperChess", 10, 27);
...
int Frame_Main::init(epdgui_args_vector_t &args) {
  ...
  // Ugly background image, don't think i want that
